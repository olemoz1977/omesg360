<?php
// PrioLens Open14 v0.2 partial-session checkpoint API.
// Stores incomplete progress under the same session_uuid used by the final API.
...
foreach (['bankSchema','planSchema','assignerSchema'] as $field) {
    if (!is_string($body[$field]) || strlen($body[$field]) > 96) fail_json(400, 'Invalid schema metadata');
}
...
try {
    ...
    error_log('PrioLens Open14 progress API error: '.$e->getMessage());
