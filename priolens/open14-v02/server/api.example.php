<?php
// PrioLens Open14 v0.2 formative pilot — DRAFT / NOT DEPLOYED
// Based on the defensive patterns already used by the existing Wave1 PHP API.
...
// Same-origin pilot endpoint. Keep this list explicit if the route changes.
...
foreach ($requiredTop as $field) {
    if (!array_key_exists($field, $body)) {
        fail_json(400, 'Missing: '.$field);
    }
}

if ($body['schema'] !== '2rasi.priolens.open14.session-v0.2') {
    fail_json(400, 'Unsupported session schema');
}
if ($body['sufficiencySchema'] !== '2rasi.priolens.sufficiency-v0.2') {
    fail_json(400, 'Unsupported sufficiency schema');
}
...
if ($seed === '' || strlen($seed) > 128) {
    fail_json(400, 'Invalid seed');
}
foreach (['startedAt','completedAt'] as $tsField) {
    if (!is_string($body[$tsField]) || strlen($body[$tsField]) > 40) {
        fail_json(400, 'Invalid timestamp');
    }
}
...
if (!is_array($body['choices']) || count($body['choices']) !== 14) {
    fail_json(400, 'Expected exactly 14 choices');
}
...
foreach ($body['choices'] as $i => $trial) {
    ...
    if (!is_string($trial['trialId']) || !preg_match('/^O14-\d{2}$/', $trial['trialId'])) {
        fail_json(400, 'Invalid trialId');
    }

    $presented = [];
    foreach ($trial['stimuli'] as $stim) {
        if (!is_array($stim) || !isset($stim['familyId'],$stim['exemplarId'],$stim['slot'])) {
            fail_json(400, 'Invalid stimulus');
        }
        ...
        if ($exemplar !== $family.'-A' && $exemplar !== $family.'-B') {
            fail_json(400, 'Invalid exemplar');
        }
    ...
    if ($noClear) {
        if (array_key_exists('choice', $trial) && $trial['choice'] !== null) {
            fail_json(400, 'noClearChoice conflicts with choice');
        }
    } else {
...
foreach ($requiredSufficiency as $key) {
    ...
    if ($v !== null && (!is_int($v) || $v < 1 || $v > 5)) {
        fail_json(400, 'Invalid sufficiency value');
    }
}

// Store canonical validated payload. Derived summaries remain recomputable.
$canonicalPayload = json_encode($body, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
if ($canonicalPayload === false || strlen($canonicalPayload) > 262144) {
    fail_json(413, 'Payload too large');
}
...
// config.php must exist on the server and must never be committed.
...
try {
    ...
    // Coarse global abuse ceiling for a small formative pilot.
    // Do not store IP merely for rate limiting. Add stronger edge/WAF controls before a public launch.
    $recentCount = (int)$pdo->query("SELECT COUNT(*) FROM priolens_open14_sessions WHERE created_at >= (NOW() - INTERVAL 1 DAY)")->fetchColumn();
    if ($recentCount > 5000) {
        fail_json(429, 'Submission limit reached');
    }
    ...
    // Idempotent retry: return the existing submission ID for the same session UUID.
