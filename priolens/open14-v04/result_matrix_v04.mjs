...
function renderInterpretationHtml(model,lang,familyLabels){
  const x=buildHumanInterpretationV04({model:model,lang:lang,familyLabels:familyLabels});
  return '<div class="interpretationHead"><div><div class="interpretationKicker">'+esc(lang==='en'?'General reflection':'Bendrinė refleksija')+'</div><h2>'+esc(x.title)+'</h2></div><span class="interpretationBoundary">'+esc(lang==='en'?'Not a diagnosis':'Ne diagnozė')+'</span></div>'
    +'<p class="interpretationDisclaimer">'+esc(x.disclaimer)+'</p>'
    +'<div class="interpretationBody">'+x.paragraphs.map(function(p){return '<p>'+richText(p)+'</p>'}).join('')+'</div>'
    +'<div class="interpretationQuestion"><span>'+esc(x.questionLabel)+'</span><strong>'+esc(x.question)+'</strong></div>';
}
...
export function renderResultMatrixV04(args){
  ...
  q('matrixInterpretation').innerHTML=renderInterpretationHtml(model,lang,familyLabels);
...
'    <section id="matrixInterpretation" class="matrixInterpretation" aria-live="polite"></section>',
