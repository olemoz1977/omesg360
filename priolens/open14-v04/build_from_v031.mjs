...
html=replaceOnce(html,"leastQ:'O kuris iš likusių traukia mažiausiai?',leastHint:'Pirmo pasirinkimo nebekeičiame. Žiūrėk tik į kitus du.'","leastQ:'Kuri iš dviejų likusių nuotraukų traukia mažiausiai?',leastHint:'Pažymėk vieną iš dviejų likusių nuotraukų.'",'LT LEAST instruction');
...
html=replaceOnce(html,"leastQ:'And which of the other two pulls you least?',leastHint:'Your first choice is fixed. Look only at the other two.'","leastQ:'Which of the two remaining photos pulls you least?',leastHint:'Select one of the two remaining photos.'",'EN LEAST instruction');
html=replaceOnce(html,"noneMost:'Nė vienas aiškiai',tieLeast:'Abu likę panašiai'","noneMost:'Nė vienas aiškiai',undoMost:'Keisti pirmą pasirinkimą',tieLeast:'Abu likę panašiai'",'LT undo label');
html=replaceOnce(html,"noneMost:'None clearly',tieLeast:'The other two feel similar'","noneMost:'None clearly',undoMost:'Change first choice',tieLeast:'The other two feel similar'",'EN undo label');
...
const clarifierCss=`
.stage{grid-template-columns:repeat(2,minmax(0,1fr))}.stim{grid-column:1/-1}.none{grid-column:1/-1}#undoMost{grid-column:1;font-size:13px;padding:8px 10px}#tieLeast{grid-column:2;font-size:13px;padding:8px 10px}.rankAssist{font-weight:600}.rankAssist:disabled{opacity:.32;cursor:default;filter:grayscale(1)}
...
html=replaceOnce(html,"$('noneMost').classList.toggle('hidden',leastPhase);$('tieLeast').classList.toggle('hidden',!leastPhase);","$('noneMost').classList.toggle('hidden',leastPhase);$('undoMost').disabled=!leastPhase;$('tieLeast').disabled=!leastPhase;",'disable assist actions until MOST');
