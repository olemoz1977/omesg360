...
try{
  ...
  if(await page.locator('#needsMapStage .mapRoutes,#needsMapStage .routePath,#needsMapStage .routeOrigin,#needsMapStage .landShape').count()) throw new Error('no-route state must not render route or land geometry');
  ...
  console.log('PASS: deployed v0.4 frozen IA + minimalist ship + cartographic map/no-route geometry guard + clean details + reload restore + isolated live API');
} finally {
  await browser.close();
}
