...
// Protocol identity: keep bank/assigner v0.3, but do not mix positive-only and bidirectional sessions.
...
// Intro copy: the second visual decision is part of the task, not a hidden telemetry field.
html=replaceN(html,
  ...
  "introLead:'Trys vaizdai vienu metu. Pirmiausia pasirink, kuris patraukia. Tada iš likusių dviejų pažymėk, kuris traukia mažiausiai.',introNote1:'14 trumpų trijulių · apie 3 min. Po jų į tą pačią situaciją pažvelgsi iš kitos perspektyvos.'",
  1,'LT rank intro copy');
html=replaceN(html,
  ...
  "introLead:'Three images at a time. First choose which one pulls you. Then, from the other two, mark which one pulls you least.',introNote1:'14 quick triads · about 3 min. After them, you will look at the same moment from a different perspective.'",
  1,'EN rank intro copy');

// Rank interaction styling. First choice stays visible but is locked during the least step.
html=replaceN(html,'\n#suff.active{',`\n.stim.most img{border:4px solid #181818}.stim.most::after{content:'PIRMAS';position:absolute;left:12px;top:8px;background:#181818;color:#fff;font-size:10px;font-weight:850;letter-spacing:.07em;padding:5px 7px;border-radius:999px}.stim{position:relative}.stim:disabled{cursor:default}.rankHint{font-size:12px;color:#666;margin-top:2px}.rankLeastNote{font-size:14px;line-height:1.5;color:#666;margin:8px 2px 0}\n#suff.active{`,1,'rank CSS insertion');

// Trial surface: two explicit phases and two phase-specific fallback controls.
html=replaceN(html,
  '<div class="trialhead"><div class="q">Kuris pirmas patraukia?</div><div id="counter" class="count">1 / 14</div></div>',
  '<div class="trialhead"><div><div id="trialQuestion" class="q">Kuris pirmas patraukia?</div><div id="trialHint" class="rankHint">Rinkis pirmą impulsą.</div></div><div id="counter" class="count">1 / 14</div></div>',
  1,'rank trial header');
html=replaceN(html,
  '<button id="none" class="none">Nė vienas aiškiai</button>',
  '<button id="noneMost" class="none">Nė vienas aiškiai</button><button id="tieLeast" class="none hidden">Abu likę panašiai</button>',
  1,'rank trial actions');

// Result hierarchy: MOST and LEAST stay distinct; LEAST is not described as unimportance.
...
// Add explicit bilingual rank copy without reopening the broader translation surface.
html=replaceN(html,
  "const T=TXT[LANG];",
  ...
  1,'rank bilingual copy object');

// Static language application must also own the two phase controls and the new result section.
html=replaceN(html,
...
html=replaceN(html,helperNeedle,helperReplacement,1,'least result helpers');

// Render LEAST repetitions independently from MOST; do not subtract them into a participant score.
...
const repeatedReplacement=repeatedNeedle+"\n  const leastMount=$('leastRows');leastMount.innerHTML='';const leastRepeated=Object.entries(state.familyStats).filter(([,x])=>x.least>=2&&x.crossLeastExemplar).sort((a,b)=>b[1].least-a[1].least);if(!leastRepeated.length)addInsight(leastMount,R.noLeast,'','');else leastRepeated.forEach(([id,x])=>addLeastChoiceInsight(leastMount,id,x));";
html=replaceN(html,repeatedNeedle,repeatedReplacement,1,'least result rendering');

// Session summary includes direction-specific timing/ties while preserving MOST metrics used by older analysis code.
html=replaceN(html,
...
for(const token of ['2rasi.priolens.open14.rank-session-v0.3','leastChoice','crossLeastExemplar','id="tieLeast"','id="leastRows"','rankProtocol']){
  if(!html.includes(token))throw new Error(`Rank build missing required token: ${token}`)
}
