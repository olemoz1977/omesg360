<?php
// PrioLens Open14 v0.3 MOST+LEAST formative API source.
// Intended deploy target: /priolens-open14-v03-api/api.php
// Does not replace the live v0.2 endpoint.
...
$sessionExemplars = [];
$familyExemplars = array_fill_keys($allowedFamilies, []);
...
foreach ($body['choices'] as $i => $trial) {
    ...
    foreach ($trial['stimuli'] as $stim) {
        ...
        if (isset($sessionExemplars[$exemplar])) fail_json(400, 'Exact exemplar repeated in session');
        $sessionExemplars[$exemplar] = true;
        $familyExemplars[$family][$exemplar] = true;
    }
    ...
    if ($noClear) {
        ...
        $leastRt = (int)$trial['leastRtMs'];
        if ($leastRt < 0 || $leastRt > 600000) fail_json(400, 'Invalid leastRtMs');
    }
...
if (count($sessionExemplars) !== 42) fail_json(400, 'Expected 42 unique presented exemplars');
foreach ($allowedFamilies as $family) {
    $ids = array_keys($familyExemplars[$family]);
    sort($ids);
    if ($ids !== [$family.'-01',$family.'-02',$family.'-03']) fail_json(400, 'Family exemplar coverage invalid: '.$family);
}
...
try {
    ...
    error_log('PrioLens Open14 v0.3 rank API error: '.$e->getMessage());
