import { calculateDirectionalBalance, NOT_ESTIMABLE } from './calculation_engine.mjs';
...
export function deriveProductResultState({ events, gateDConfig, gateEConfig, reflectionAnchors = [] } = {}) {
  ...
  const directionalResultAvailable = ['CS', 'CR'].some(domain =>
    domains[domain].calculation.directionBalance !== NOT_ESTIMABLE &&
    domains[domain].evidence.allowedClaimLevel > 0
  );

  const gateDReady = Array.isArray(gateDConfig?.mappings) &&
    gateDConfig.mappings.some(mapping => mapping.mapping_status === 'VALIDATED');
  const gateEReady = ['CS', 'CR'].some(domain => gateEConfig?.domains?.[domain]?.status === 'VALID');

  return {
    resultStatus: directionalResultAvailable ? 'LIMITED_RESULT_AVAILABLE' : 'NOT_ESTIMABLE',
    directionalResultAvailable,
    gateDReady,
    gateEReady,
    context,
    domains,
